...
def add_annotations(snv_papa_results_file, clinvar_info, acc2common, plaac_prlds):
    ...
    output.write('Accession\tCommon Protein Name\tMutation\tHighest PAPA score\tPosition of Highest Scoring Window\tOriginal PAPA score\tMutant PAPAscore - Original PAPAscore\tCrosses 0.0 Threshold?\tCrosses 0.05 Threshold?\tPhenotype/Disease\tOriginal Sequence\tMutant Sequence\n')

    fig3a_plotting_df = { 'Accessions':[],
                    'Common names':[],
                    'Orig scores':[],
                    'Mutant scores':[] }
                    
    fig3b_plotting_df = { 'Accessions':[],
                    'Common names':[],
                    'Orig scores':[],
                    'Mutant scores':[] }
    ...
    for line in h:
        ...
        if '_' not in accession.replace('NM_', ''):
            ...
            if orig_PAPAscore < 0.05 and float(papa_score) > 0.05:
                fig3a_plotting_df['Accessions'].append(acc)
                fig3a_plotting_df['Common names'].append(common_name)
                fig3a_plotting_df['Orig scores'].append(orig_PAPAscore)
                fig3a_plotting_df['Mutant scores'].append( float(papa_score) )
                
            if orig_PAPAscore > 0.05 and float(papa_score) > orig_PAPAscore:
                fig3b_plotting_df['Accessions'].append(acc)
                fig3b_plotting_df['Common names'].append(common_name)
                fig3b_plotting_df['Orig scores'].append(orig_PAPAscore)
                fig3b_plotting_df['Mutant scores'].append( float(papa_score) )
...
def plot_fig3a(small_df, plaac_positives):
    ...
    for i in range(len(small_df['Common names'])):
        plt.bar(x=i, height=current_iso_PAPAscore[i], color=palette[0])
        if current_iso_PAPAscore[i] < 0:
            b = 0
            plt.bar(x=i, height=max_iso_PAPAscore[i], bottom=b, color=palette[1])
        else:
            b = current_iso_PAPAscore[i]
            plt.bar(x=i, height=(max_iso_PAPAscore[i] - current_iso_PAPAscore[i]), bottom=b, color=palette[1])
    ...
    plt.savefig('Fig 3a.tiff', bbox_inches='tight', dpi=600)
    plt.close()
...
def main(snv_papa_results_file, snv_plaac_results_file, snv_info_file):
    ...
    fig3a_plotting_df, fig3b_plotting_df = add_annotations(snv_papa_results_file, snv_info, acc2common, plaac_boundaries)
...
if __name__ == '__main__':
    ...
    #snv_plaac_results_file --> 'Human_Protein_Seqs_ActiveDriverDB_PLAAC_results' (the result of running plaac.jar on 'Human_Protein_Seqs_ActiveDriverDB.FASTA' with alpha=0 and the FASTA file used to calculate background amino acid frequencies)
