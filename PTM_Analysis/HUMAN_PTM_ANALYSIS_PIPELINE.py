...
def add_PTM_and_PLAAC_annotations(papa_results_file, plaac_results_file, mod_dict):
    ...
    output.write('Accession Number\tGene\tMax PAPA Score\tHighest-scoring Window Sequence(s)\tHighest-scoring Window(s) Indices\tPTMs within High-scoring Domain(s)\tOverlapping PLAAC PrLD?\tPLAAC PrLD Indices\n')
...
def barchart_lnOR_PTMs(mod_info):
    ...
    for i in range(len(lnORs)):
        ...
        labels.append(mod)
        if lnOR > 0:
            plt.bar(i, lnOR, color='xkcd:sky blue', yerr=standard_error, ecolor='black')
        else:
            plt.bar(i, lnOR, color='xkcd:scarlet', yerr=standard_error, ecolor='black')
